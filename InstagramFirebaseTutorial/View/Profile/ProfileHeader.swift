...
class ProfileHeader: UICollectionReusableView {
    ...
    private lazy var postLabel: UILabel = {
        ...
        label.attributedText = attributedStatText(value: 1, label: "게시물")
        return label
    }()
    ...
    private lazy var followersLabel: UILabel = {
        ...
        label.numberOfLines = 0
        label.textAlignment = .center
        label.attributedText = attributedStatText(value: 1, label: "팔로워")
        return label
    }()
    ...
    @objc func handleEditProfileFollowTapped() {
        print("디버그: 프로필 편집 버튼 확인")
    }
    
    // MARK: - 도우미
    func attributedStatText(value: Int, label: String) -> NSAttributedString {
        let attributedText = NSMutableAttributedString(string: "\(value)\n", attributes: [.font: UIFont.boldSystemFont(ofSize: 14)])
        attributedText.append(NSAttributedString(string: label, attributes: [.font: UIFont.boldSystemFont(ofSize: 14), .foregroundColor: UIColor.lightGray.cgColor]))
